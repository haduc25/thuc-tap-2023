...
const Admin = () => {
    ...
    const [secondsRemaining, setSecondsRemaining] = useState(5); // Thời gian mặc định là 5 giây
    ...
    useEffect(() => {
        if (currentUser.role !== 'admin') {
            ...
            // Hiển thị thông báo khi không phải là admin
            setShowAccessDenied(true);
            ...
            // Clear interval khi không còn cần thiết
            const intervalId = setInterval(() => {
                setSecondsRemaining((prevSeconds) => prevSeconds - 1);
            }, 1000);
        ...
        <div>
            {showAccessDenied ? (
                <div>
                    ...
                    {/* Thêm hình ảnh và nút quay lại trang chủ tại đây */}
                    <button onClick={() => navigate('/')}>Quay lại trang chủ</button>
                </div>
            ) : (
                <div>
                    <p>Mày là admin đấy bro</p>
                </div>
            )}
        </div>
    );
